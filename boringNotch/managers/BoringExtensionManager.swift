...
let newHudExtension = Extension(
    name: hudExtension.components(separatedBy: ".").last ?? hudExtension,
    bundleIdentifier: hudExtension,
    status: .enabled
)
...
class BoringExtensionManager: ObservableObject {
    ...
    @objc func checkIfExtensionsAreInstalled() {
        installedExtensions = []
        //installedExtensions.append(newHudExtension) // oori added
