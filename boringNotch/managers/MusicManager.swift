...
class MusicManager: ObservableObject {
    ...
    private let MRMediaRemoteGetNowPlayingApplicationIsPlaying:
    ...
    private func setupNowPlayingObserver() {
        ...
        observeNotification(name: "kMRMediaRemoteNowPlayingInfoDidChangeNotification") {
            [weak self] in
            self?.fetchNowPlayingInfo(bundle: self?.nowPlaying.appBundleIdentifier ?? nil)
        }

        observeNotification(name: "kMRMediaRemoteNowPlayingApplicationDidChangeNotification") {
            [weak self] in
            self?.updateApp()
        }

        observeDistributedNotification(name: "com.spotify.client.PlaybackStateChanged") {
            [weak self] in
        ...
        detector.$currentAppInFullScreen
            .sink { [weak self] _ in
                self?.fetchNowPlayingInfo(
                    bypass: true, bundle: self?.nowPlaying.appBundleIdentifier ?? nil)
            }
    ...
    @objc func updateApp() {
        bundleIdentifier = nowPlaying.appBundleIdentifier ?? "com.spotify.client"  //changed from com.apple.Music <><><> Oori.S
    }
    ...
    @objc func fetchNowPlayingInfo(bypass: Bool = false, bundle: String? = nil) {
        ...
        MRMediaRemoteGetNowPlayingInfo(DispatchQueue.main) { [weak self] information in
            ...
            guard
                let elapsedTime = information["kMRMediaRemoteNowPlayingInfoElapsedTime"]
                    as? TimeInterval,
                let timestampDate = information["kMRMediaRemoteNowPlayingInfoTimestamp"] as? Date,
                let playbackRate = information["kMRMediaRemoteNowPlayingInfoPlaybackRate"]
                    as? Double
            else {
                return
            }
    ...
    private func startElapsedTimeTimer() {
        stopElapsedTimeTimer()
        elapsedTimeTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) {
            [weak self] _ in
            guard let self = self else { return }
            if self.isPlaying {
