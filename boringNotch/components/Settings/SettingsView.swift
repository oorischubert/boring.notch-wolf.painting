                        ...
                        HStack {
                            ...
                            Menu(content: {
                                ...
                                Button("Disable") {
                                    ...
                                    if let ext = ws.runningApplications.first(where: { $0.bundleIdentifier == item.bundleIdentifier }) {
                                        // Force terminate to ensure the app quits
                                        ext.Terminate()
                                        
                                        // Wait briefly to let termination complete, then update status
                                        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                                            if ext.isTerminated {
                                                extensionManager.installedExtensions[index].status = .disabled
                                                print("Extension disabled and terminated.")
                                            } else {
                                                print("Failed to disable extension.")
                                            }
                                        }
                                    } else {
                                        // If it's not running, just mark as disabled.
                                        extensionManager.installedExtensions[index].status = .disabled
                                        print("Extension already not running; marked as disabled.")
                                    }
                                }
